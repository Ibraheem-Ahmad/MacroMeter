...
class MacroApp:
    ...
    def capture_and_calculate(self):
        ...
        # 7. Save to JSON file
        try:
            # Go up 2 levels: AIMacroCalc/ → MacroMeter/
            macro_meter_folder = pathlib.Path(__file__).resolve().parents[2]

            # From there, go into src/data/dish_macros.json
            output_path = macro_meter_folder / "src" / "data" / "dish_macros.json"
            ...
            print(f"dish_macros.json updated at {output_path}")
